...
// Lookup table for possible frequencies
...
// Introducing a voltage table for all available frequencies and ASV levels instead of only for the 5 standard steps
...
mali_bool mali_dvfs_table_update(void)
{
	unsigned int step_num = MALI_DVFS_STEPS;
        ...
        // Update voltage for all 5 freq. steps based on true freq. table
        for (i = 0; i < step_num; i++) {

                for (j = 0; gpu_freq_table[j] != GPU_FREQ_END_OF_TABLE; j++) {

                        if (gpu_freq_table[j] == mali_dvfs[i].clock) { // if we have found the right freq. step, use that voltage
