...
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG1	(50)	//  default for hotplug up threshold for cpu1 (cpu0 stays allways on)
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG2	(70)	//  default for hotplug up threshold for cpu2 (cpu0 stays allways on)
#define DEF_FREQUENCY_UP_THRESHOLD_HOTPLUG3	(98)	//  default for hotplug up threshold for cpu3 (cpu0 stays allways on)
#define DEF_FREQUENCY_DOWN_THRESHOLD		(40)
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG1	(35)	//  default for hotplug down threshold for cpu1 (cpu0 stays allways on)
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG2	(45)	//  default for hotplug down threshold for cpu2 (cpu0 stays allways on)
#define DEF_FREQUENCY_DOWN_THRESHOLD_HOTPLUG3	(65)	//  default for hotplug down threshold for cpu3 (cpu0 stays allways on)
#define DEF_IGNORE_NICE				(0)	//  default for ignore nice load
#define DEF_FREQ_STEP				(10)	//  default for freq step on awake
#define DEF_FREQ_STEP_SLEEP			(1)	//  default for freq step on early suspend
...
#define SAMPLING_RATE_SLEEP_MULTIPLIER 		(2)	//  default for tuneable sampling_rate_sleep_multiplier
#define DEF_UP_THRESHOLD_SLEEP			(100)	//  default for tuneable up_threshold_sleep
#define DEF_DOWN_THRESHOLD_SLEEP		(60)	//  default for tuneable down_threshold_sleep
#define DEF_SMOOTH_UP_SLEEP			(100)	//  default for tuneable smooth_up_sleep

/*
* Hotplug Sleep: 0 do not touch hotplug settings on early suspend, so all cores will be online
* values 1, 2 or 3 are equivalent to cores which should be online on early suspend
*/

#define DEF_HOTPLUG_SLEEP			(1)	//  default for tuneable hotplug_sleep
...
#define DEF_FREQ_LIMIT_SLEEP			(1)	//  default for tuneable freq_limit_sleep

/*
* Fast Scaling: 0 do not activate fast scaling freqencies
* value 1 to enable fast scaling freqencies
*/

#define DEF_FAST_SCALING			(1)	//  default for tuneable fast_scaling
#define DEF_FAST_SCALING_SLEEP			(1)	//  default for tuneable fast_scaling_sleep
...
static struct dbs_tuners {
	...
	.sampling_rate_sleep_multiplier = SAMPLING_RATE_SLEEP_MULTIPLIER,	//  set default value for new tuneable
...
//  up/down switches for fast scaling columns
#define IM_UP 3
#define IM_DOWN 4
...
show_one(sampling_rate_sleep_multiplier, sampling_rate_sleep_multiplier);	//  added smapling_rate_sleep_multiplier tuneable for early suspend
...
//  added tuneable -> possible values: 1 or 2, if not set default is 2
...
/* 
 * added tuneable -> possible values: 0 disable, 200000-1600000 khz -> freqency soft-limit, if not set default is 0
...
/* 
 * added tuneable -> possible values: 0 disable, 200000-1600000 khz -> freqency soft-limit on early suspend, if not set default is 0
 */
...
static void dbs_check_cpu(struct cpu_dbs_info_s *this_dbs_info)
{
	...
	if (num_online_cpus() < 2) {
			if (dbs_tuners_ins.up_threshold_hotplug1 != 0 || dbs_tuners_ins.up_threshold_hotplug2 != 0 || dbs_tuners_ins.up_threshold_hotplug3 != 0) //  don't mutex if cores are not enabled
			...
			if (dbs_tuners_ins.up_threshold_hotplug2 != 0 || dbs_tuners_ins.up_threshold_hotplug3 != 0) //  don't mutex if cores are not enabled
			mutex_unlock(&this_dbs_info->timer_mutex); //  this seems to be a very good idea, without it lockups are possible!
			if (dbs_tuners_ins.up_threshold_hotplug2 != 0 && max_load > dbs_tuners_ins.up_threshold_hotplug2)
			cpu_up(2);
			if (dbs_tuners_ins.up_threshold_hotplug3 != 0 && max_load > dbs_tuners_ins.up_threshold_hotplug3)
			...
			if (dbs_tuners_ins.up_threshold_hotplug3 != 0) //  don't mutex if cores are not enabled
			mutex_unlock(&this_dbs_info->timer_mutex); //  this seems to be a very good idea, without it lockups are possible!
			if (dbs_tuners_ins.up_threshold_hotplug3 != 0 && max_load > dbs_tuners_ins.up_threshold_hotplug3)
			cpu_up(3);
			if (dbs_tuners_ins.up_threshold_hotplug3 != 0) //  don't mutex if cores are not enabled
	...
	/* Frequency Limit: but let it scale up as normal if the freqencies are lower freq_limit */
	    ...
	    if (dbs_tuners_ins.fast_scaling == 1) //  use fast scaling columns
		this_dbs_info->requested_freq = mn_get_next_freq(policy->cur, IM_UP, max_load);
	    else
	...
	/* Frequency Limit: else we scale down as usual */
	    ...
	    if (dbs_tuners_ins.fast_scaling == 1) //  use fast scaling columns
		this_dbs_info->requested_freq = mn_get_next_freq(policy->cur, IM_DOWN, max_load);
	    else
 ...
 * Source Code zzmoove 0.4: https://github.com/zanezam/cpufreq-governor-zzmoove/commit/6ba865e5fb0e3f4345fe00fa0d5e9851da9c9897 "
 *
